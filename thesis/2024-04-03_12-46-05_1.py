...
class Model(nn.Module):
    ...
    def forward(self, x, mask, cls_headers, question_len):
        ...
        for i in range(batches):
            ...
            cat_q_h = F.softmax(cat_q_h, dim=-1)
...
def train(model, train_data_loader, test_data_loader):
    ...
    for epoch in range(1, Config.epochs + 1):
        ...
        log.info(f'epoch={epoch}, loss={loss / len(train_data_loader.dataset)} train_acc={train_acc:.4%}, test_acc={test_acc:.4%}')
...
def valid(model, dataloader):
    ...
    log.info(f'agg_acc={agg_cnt}/{dataset_size}, 
               cond_conn_op_acc={cond_conn_op_cnt}/{dataset_size},
               cond_ops_acc={cond_ops_cnt}/{dataset_size},
               cond_cols_acc={cond_cols_cnt}/{dataset_size},
               total_acc={total}/{len(dataloader.dataset)}')
...
def main():
    ...
    global optim
    global scheduler
    optim = torch.optim.Adam(model.parameters(), lr=Config.lr, weight_decay=1e-8)
