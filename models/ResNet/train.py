...
import models.ResNet.config as config
from log import log
...
def train(model, train_data_loader, test_data_loader):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % config.step == 0:
            LOG.info(
                f'epoch={epoch} loss={(metric[0] / metric[2]):.5f}, train acc={(metric[1] / metric[2] * 100):.5f} test acc={val_metric * 100:.3f}')

    LOG.info(
        f'{metric[2] * epochs / timer.sum():.1f} examples/sec on {str(device)}')
...
def main():
    ...
    train_iter, test_iter = d2l.load_data_fashion_mnist(
        batch_size=config.hyper_params['batch_size'], resize=96)
    model = Model()
